...
- Audio path: `AVAudioEngine` input tap → `AVAudioConverter` → 16 kHz mono Float32; samples buffered under `NSLock` (audio thread), RMS → published `level` for the waveform. `WhisperTranscriber` caches the whisper context across sessions on a private serial queue; segment timestamps are centiseconds (×10 = ms).
- Live streaming: `DictationStreamEngine` transcribes ~1.6s chunks with 700ms overlap while recording; segments ending ≥700ms before the window end are finalized (never revised), the rest stay provisional (`livePartialText`, dimmed in the pill). On stop, only the un-finalized tail runs (seeded with finalized text as `initial_prompt`). RMS-quiet windows skip the GPU run. Stitch logic is pure (`nonisolated static stitch`) and was verified against a single full run via a harness.
